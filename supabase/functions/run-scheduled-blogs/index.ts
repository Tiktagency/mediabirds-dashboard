...
Deno.serve(async (req) => {
  ...
  try {
    ...
    for (const schedule of dueSchedules) {
      ...
      if (!company) {
        console.error(`No company found for blog schedule ${schedule.id}`);
        continue;
      }
      ...
      try {
        ...
        // Call trigger-blog-generation edge function (same as manual button)
        console.log(`Calling trigger-blog-generation for ${company.name}`);
        
        const webhookResponse = await fetch(`${supabaseUrl}/functions/v1/trigger-blog-generation`, {
          method: 'POST',
          headers: {
          ...
          body: JSON.stringify({
            webhookUrl: FIXED_BLOG_WEBHOOK_URL,
            ...
            blogData: {
              ...
              taal: blogSettings.taal || 'Nederlands',
        ...
        console.log(`trigger-blog-generation response for ${company.name}:`, responseText);
        ...
        if (triggerSuccess) {
          ...
          if (updateError) {
            console.error(`Error updating blog schedule for ${company.name}:`, updateError);
          }
