...
export const useAltTextSchedule = () => {
  const [schedule, setSchedule] = useState<AltTextSchedule | null>(null);
  ...
  useEffect(() => {
    const fetchSchedule = async () => {
      ...
      if (error) {
        console.error('Error fetching alt text schedule:', error);
        setSchedule(null);
      } else {
        setSchedule(data as AltTextSchedule | null);
      }
      setIsLoading(false);
    };
    fetchSchedule();
  ...
  const updateSchedule = async (updates: UpdateScheduleData) => {
    ...
    try {
      if (schedule) {
        ...
        setSchedule(prev => prev ? { ...prev, ...updateData } as AltTextSchedule : null);
      } else {
        const { data, error } = await supabase
          .from('alt_text_schedules')
          .insert(updateData)
          ...
          .single();
        if (error) throw error;
        setSchedule(data as AltTextSchedule);
      }
      ...
      console.error('Error updating alt text schedule:', error);
