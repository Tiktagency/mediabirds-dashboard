...
export const useLandingSchedule = (companyId: string | null | undefined) => {
  ...
  useEffect(() => {
    ...
    const fetchSchedule = async () => {
      ...
      if (error) {
        console.error('Error fetching landing schedule:', error);
        setSchedule(null);
      } else {
        setSchedule(data as LandingSchedule | null);
      }
      setIsLoading(false);
    };
    fetchSchedule();
  ...
  const updateSchedule = async (updates: UpdateScheduleData) => {
    ...
    try {
      if (schedule) {
        ...
        setSchedule(prev => prev ? { ...prev, ...updateData } as LandingSchedule : null);
      } else {
        const { data, error } = await supabase
          .from('landing_schedules')
          .insert({
          ...
          .single();
        if (error) throw error;
        setSchedule(data as LandingSchedule);
      }
      ...
      console.error('Error updating landing schedule:', error);
