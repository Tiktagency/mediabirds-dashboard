...
import { Bell, X, Search, FileText, Link as LinkIcon, BookOpen, Pencil, Save } from 'lucide-react';
...
const SeoBlog = () => {
  ...
  const handleSaveContent = async () => {
    ...
    const { error } = await supabase
      .from('app_settings')
      .update({ value: editedContent.trim(), updated_at: new Date().toISOString() })
      .eq('key', 'seo_guide_content');
    
    if (!error) {
      setGuideContent(editedContent.trim());
          ...
          {isEditingGuide ? (
            <div className="mt-6 space-y-4">
              <textarea
                ...
                className="w-full h-[500px] bg-white/10 border border-white/20 rounded-lg p-4 text-white text-sm font-mono resize-none focus:outline-none focus:border-white/40"
              ...
              <div className="flex gap-2">
                ...
                >
                  ...
                  {isSavingContent ? 'Opslaan...' : 'Opslaan'}
                </Button>
                <Button 
                  variant="ghost" 
            ...
            <div className="mt-6 prose-guide pb-8">
